...
for idx, (string, color) in enumerate(filtered_strings.items()):
    # Bar for dataset 1
    ax.bar('Feces_Control', sorted_counts1[idx], bottom=bottom1, color=color, edgecolor="black", label=string, width=bar_width)
    bottom1 = [bottom1[i] + sorted_counts1[idx] for i in range(len(bottom1))]

    # Bar for dataset 2
    ax.bar('Feces_Infected', sorted_counts2[idx], bottom=bottom2, color=color, edgecolor="black", width=bar_width)
